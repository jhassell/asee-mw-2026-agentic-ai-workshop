...
**Workshop takeaway (b): a starter assignment adaptable to your own course.**

From *From Chatbots to Agents*, ASEE Midwest Section 2026 (Workshop 9).
John Hassell, OU Polytechnic Institute.
Released MIT — copy it, cut it, rename it, put your own name on it. No
attribution required, no correspondence needed.
...
an AI assistant, and grade the supervision instead of the output.**
...
difference, and it cannot warn you.
...
> *You have a folder of 79 conference papers. Produce a one-page evidence
> brief: how many of these papers are about AI in engineering education, which
> ones, and which of those report a specific measured number about a student
> or instructor outcome — with the number, as stated.*
...
open literature — provide the corpus, so verification is possible.
...
**2. The duplicate.** Take a real item, copy it, change only its identifier,
and place it far from the original in whatever ordering students will use —
far enough that the two never appear on the same screen. Identical title,
identical authors, identical body. Leave the original's ID printed inside the
body, so the copy contradicts itself for anyone who opens it. It adds one to
...
- **Remove your own shortcuts from the student packet.** If you have an index,
  ...
  the trap without knowing. Ship the material; keep the index.
...
- **99.3% of the tokens were input, not output.** Agents read enormously more
  ...
  a small corpus, or hand them a large one and require them to filter it
...
- It does not teach anyone to build an agent. It teaches supervision of one.
  Those are different courses; this is the first hour of the second one.
